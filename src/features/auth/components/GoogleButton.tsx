...
export default function GoogleButton() {
	// Googleボタンに認証ページへの遷移を追加
	useEffect(() => {
		const googleLoginButton = document.getElementById('gsi-material-button') as HTMLButtonElement;
		if (!googleLoginButton) return;
		googleLoginButton.addEventListener('click', RequestToken);

		return () => {
			googleLoginButton.removeEventListener('click', RequestToken);
		};
	}, []);

	return (
		<div className='flex justify-center items-center h-60v'>
			<button className={styles["gsi-material-button"]} id='gsi-material-button' >
