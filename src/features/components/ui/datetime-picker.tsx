...
import { CalendarProps } from '@/features/components/ui/calendar';
...
function Calendar({
	...
	const MONTHS = React.useMemo(() => genMonths(props.locale || ja), []);
	const YEARS = React.useMemo(() => genYears(props.locale || ja, yearRange), []);
...
const DateTimePicker = React.forwardRef<DateTimePickerRef, DateTimePickerProps>(
	(
		...
		const initHourFormat = {
			...
			hour12: displayFormat?.hour12 ?? 'PPP',
		};
